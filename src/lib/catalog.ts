...
// Stores the inspected label photograph and returns a viewable link for it.
export const uploadLabelPhoto = async (file: File, code: string) => {
  const key = `${code || "unassigned"}/${Date.now()}-${file.name.replace(/[^\w.-]+/g, "-")}`;
