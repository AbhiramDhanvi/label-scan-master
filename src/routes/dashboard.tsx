  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <main className="mx-auto w-full max-w-5xl px-4 py-8">
        ...
        <section className="mt-6 bg-card outline outline-border">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border px-4 py-2.5">
            <h2 className="font-mono text-xs font-semibold">PRODUCTS</h2>
            <div className="flex flex-wrap items-center gap-2">
              <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search product" className="w-40 bg-background px-2.5 py-1 text-[12px] outline outline-border" />
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-[13px]">
              <thead><tr className="border-b border-border text-left font-mono text-[10px] uppercase text-muted-foreground"><th className="px-4 py-2">Product</th><th className="px-3 py-2">Net qty</th><th className="px-3 py-2">Flagged</th><th className="px-4 py-2">Status</th></tr></thead>
              <tbody>{rows.map((p) => (
                <tr key={p.code} className="border-b border-border/70 align-top hover:bg-background">
                  ...
                  <td className="px-3 py-3 text-[12px] text-muted-foreground">{p.flagged.length === 0 ? "—" : p.flagged.map((c) => label(c)).join(", ")}</td>
                  <td className={`px-4 py-3 font-mono text-[11px] font-semibold ${verdictClass(p.verdict)}`}>{p.verdict}</td>
                </tr>
              ))}
              {rows.length === 0 && <tr><td colSpan={4} className="px-4 py-8 text-center text-xs text-muted-foreground">{productsQuery.isLoading ? "Loading products." : "No matching products."}</td></tr>}
              </tbody>
            </table>
          </div>
        </section>
              ...
              <li key={r.id} className="flex flex-wrap items-center justify-between gap-3 px-4 py-2.5">
                ...
                {r.state === "PENDING" ? (
                  <span className="flex gap-2">
                    <button className="px-2 py-1 font-mono text-[10px] outline outline-border hover:bg-muted" onClick={() => void mark(r.id, "RESOLVED")}>RESOLVED</button>
                  ...
                  <span className={`font-mono text-[10px] ${r.state === "RESOLVED" ? "text-pass" : "text-destructive"}`}>{r.state}</span>
                )}
              </li>
            ))}
            </ul>
