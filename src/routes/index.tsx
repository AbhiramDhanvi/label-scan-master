...
import { fetchLimits, fetchProducts, recordInspection, uploadLabelPhoto } from "@/lib/catalog";

export const Route = createFileRoute("/")({
  head: () => ({ meta: [
    { title: "New Inspection | Metrograph" },
    { name: "description", content: "Upload a real product label photograph, record batch details and compare the printed declarations against the legal limits of the Packaged Commodities Rules, 2011." },
    { property: "og:title", content: "New Inspection | Metrograph" },
    { property: "og:description", content: "Upload a label photograph and compare the printed declarations against the 2011 legal limits." },
...
function Index() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setReading(value);
      const code = batch.productCode || value.productCode || "";
      setBatch((prev) => ({ ...prev, productCode: prev.productCode || value.productCode || "" }));

      const outcome = evaluateLabel(value, {
      ...
      if (file) {
        try {
          photoUrl = await uploadLabelPhoto(file, code);
        } catch {
          photoUrl = null;
        }
      }
      if (code && catalog.data?.some((p) => p.code === code)) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <main className="mx-auto max-w-6xl px-5 py-10">
        ...
        <p className="mt-3 max-w-2xl text-sm text-muted-foreground">Upload the photograph of the label you are inspecting, record the batch details, then read the printed declarations from that photograph and compare them against the stored legal limits.</p>

        <div className="mt-8 grid grid-cols-1 gap-5 lg:grid-cols-12">
          <section className="bg-card outline outline-border lg:col-span-5">
            <Head title="LABEL PHOTOGRAPH" meta={file ? file.name.slice(0, 28).toUpperCase() : "NO FILE"} />
            <div className="p-4">
              <label className="grid aspect-[16/9] cursor-pointer place-items-center overflow-hidden bg-background outline outline-border hover:bg-muted">
                {imageUrl ? (
                  <img src={imageUrl} alt={`Label photograph for ${batch.productCode || "the inspected package"}`} className="h-full w-full object-contain" />
                ) : known?.labelImageUrl ? (
                  <img src={known.labelImageUrl} alt={`Stored label photograph for ${known.product}`} className="h-full w-full object-contain" />
                ) : (
                  <span className="flex items-center gap-2 font-mono text-[10px] text-muted-foreground"><Upload className="size-4" />SELECT THE LABEL PHOTOGRAPH</span>
                )}
                <input className="sr-only" type="file" accept="image/*" onChange={onImage} />
              </label>
              {known?.labelImageUrl && !imageUrl && <p className="mt-2 font-mono text-[10px] text-muted-foreground">SHOWING THE PHOTOGRAPH STORED FOR {known.code}</p>}
              ...
              {saved && <p className="mt-3 text-[13px] text-muted-foreground">{saved}</p>}
          ...
          <div className="flex flex-col gap-5 lg:col-span-7">
            <section className="bg-card outline outline-border">
              ...
              {reading ? (
                ...
                <p className="px-4 py-8 text-center text-xs text-muted-foreground">Upload the label photograph and submit the inspection to read the printed declarations.</p>
              )}
            </section>
